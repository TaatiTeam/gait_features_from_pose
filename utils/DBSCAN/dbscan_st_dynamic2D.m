function [labels] = dbscan_st_dynamic2D(points, times, height, eps_spatial, eps_temporal, min_pts)
...
eps_spatial_list = eps_spatial * height;
...
for idx_pt = 1:n_points
    eps_spatial_t = eps_spatial_list(idx_pt);
    if labels(idx_pt) ~= 0
        % Only unlabelled points can be considered as seed points.
        continue
    end
    
    set_neighbours = region_query_st(D_spatial, D_temporal, eps_spatial_t, eps_temporal, idx_pt);
    ...
    if length(set_neighbours) < min_pts
        ...
        labels = grow_cluster_st(D_spatial, D_temporal, labels, set_neighbours, label_cluster, eps_spatial_t, eps_temporal, min_pts);
